...
export default function IncomeDiscountStats() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchIncomeDiscount({ from: query.from, to: query.to });
      setData(res);
    } catch (e) {
  ...
  const handleExport = async () => {
    ...
    try {
      await exportAoaToXlsx([header, ...body], {
        filename: `收入优惠统计_${query.from}_${query.to}.xlsx`,
        sheetName: '收入优惠统计',
      });
  ...
  return (
    <div className="page-content">
      <div className="page-head">
        <h1 className="page-title">收入优惠统计</h1>
        <div className="page-head-right">
          <FavStar viewKey="rpt:income-discount" />
